...
function FAQItem({
  ...
  return (
    <div className="border border-navy/5 rounded-2xl overflow-hidden">
      <button
        onClick={onToggle}
      ...
      >
        ...
        <motion.div
          ...
          className="shrink-0"
        >
          <ChevronDown className="w-5 h-5 text-slate-warm" />
        </motion.div>
      </button>
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            transition={{ duration: 0.3 }}
          >
            <div className="px-6 pb-6 text-sm text-slate-warm leading-relaxed">
              {answer}
            </div>
          </motion.div>
        )}
